...
data "http" "vault_ca_public_key" {
  ...
  depends_on = [vault_ssh_secret_backend_ca.ssh_ca, data.http.vault_ca_check]
}
...
resource "vault_ssh_secret_backend_role" "aap" {
  ...
  depends_on = [vault_mount.ssh, vault_ssh_secret_backend_ca.ssh_ca, data.http.vault_ca_check]
}
